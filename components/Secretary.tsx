...
const Secretary: React.FC<SecretaryProps> = ({ patients, appointments, doctorName }) => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-12rem)] md:h-[calc(100vh-12rem)] bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      {/* Header */}
      <div className="px-4 md:px-6 py-3 md:py-4 border-b flex items-center gap-3 bg-slate-900 text-white shrink-0">
        <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-cyan-400 flex items-center justify-center animate-pulse">
           <svg className="w-4 h-4 md:w-5 md:h-5 text-slate-900" fill="currentColor" viewBox="0 0 20 20"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" /></svg>
        </div>
        <div>
          <h3 className="font-bold text-sm md:text-base">Rubia Assistant</h3>
          <p className="text-[9px] md:text-[10px] uppercase tracking-wider text-cyan-300">Inteligência Ativa</p>
        </div>
      </div>
      ...
      <div className="p-3 md:p-4 bg-white border-t shrink-0">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-slate-100 border-none rounded-xl px-4 py-2.5 text-xs md:text-sm focus:ring-2 focus:ring-slate-900"
          />
          <button 
            onClick={handleSend}
            disabled={isTyping || !input.trim()}
            className="bg-slate-900 text-white p-2.5 rounded-xl hover:bg-slate-800 transition-colors disabled:opacity-30 flex items-center justify-center shrink-0"
          >
